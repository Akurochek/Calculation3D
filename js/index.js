...
function createOpening(hash, type, color, scene, start, number) {
    ...
    for (let i = 0; i < massiveOpening1.length; i++) {
        ...
        if(massiveOpening5[i]>maxpositionheight){
            maxpositionheight=massiveOpening5[i]
        }
        ...
        switch (parseInt(massiveOpening1[i])) {
            case 1:
                ...
                if(Math.abs(posX - hash[0] / 2)>maxpositionlength){
                    maxpositionlength=Math.abs(posX - hash[0] / 2)
                }
                break;
            case 2:
                ...
                if(Math.abs(posX - hash[1]/2)>maxpositionwidth){
                    maxpositionwidth=Math.abs(posY - hash[1] / 2)
                }
                break;
            case 3:
                opening.position.set(-posX - massiveOpening4[i] / 2 + hash[0] / 2, posY, -By - 0.026);
            ...
            case 4:
                ...
                if(Math.abs(-posX + hash[1] / 2)>maxpositionwidth){
                    maxpositionwidth=Math.abs(-posX + hash[1] / 2)
                }
                break;
            default:
                break;
...
console.log("kds", maxpositionlength, maxpositionwidth)
...
function zinput2(){
    ...
    var digitPattern = /\d/;
    if (digitPattern.test(this.value)){
    ...
    if(document.querySelector('.input2').value<0){
        alert("ошибка")
        document.querySelector('.input2').value=lastontherightopening
...
function zinput1(){
    ...
    if(document.querySelector('.input1').value>Az1-door.scale.y){
        alert("ошибка")
        document.querySelector('.input1').value=lastupopening
...
function zinput3(){
    ...
    console.log(door.userData.side, door.userData.side=="запад" || door.userData.side=="юг")
    if((door.userData.side=="запад" || door.userData.side=="восток") && (door.position.x-(document.querySelector('.input3').value/2)+Bx<0 || door.position.x+(document.querySelector('.input3').value/2)>Bx)){
        alert("ошибка")
        document.querySelector('.input3').value=lastheightopening
        
    }else if((door.userData.side=="север" || door.userData.side=="юг") &&(door.position.z-(document.querySelector('.input3').value/2)+By<0 || door.position.z+(document.querySelector('.input3').value/2)>By)){
        alert("ошибка")
        document.querySelector('.input3').value=lastheightopening
    }else{
        lastheightopening=document.querySelector('.input3').value
    ...
    const door = scene.children[zip]
        ...
        if(door.userData.type=="дверь"){
            if(document.querySelector('.input4').value*1>Az1){
                alert("ошибка")
                document.querySelector('.input4').value=lastwidthopening
            }else{
                lastwidthopening=document.querySelector('.input4').value
            ...
            if(document.querySelector('.input4').value/2+door.position.y*1>Az1){
                alert("ошибка")
                document.querySelector('.input4').value=lastwidthopening
            }else if(door.position.y-document.querySelector('.input4').value/2<0){
                alert("ошибка")
                document.querySelector('.input4').value=lastwidthopening
            }else{
                lastwidthopening=document.querySelector('.input4').value
...
let nowNumber=""
let lastupopening=0
let lastontherightopening=0
let lastwidthopening=0
let lastheightopening=0
function openings(draggable){
    ...
    document.querySelector(".cover8").style.display = "block"
    ...
    nowType=draggable.userData.type
    nowNumber=draggable.userData.number
...
document.querySelector('.input5').addEventListener("change", (event) => {
    ...
    if (digitPattern.test(length) && length>0 && maxpositionlength*1<=l){
        lastlength=length
    ...
    for (let i = 0; i < scene.children.length; i++) {
        ...
        document.querySelector('.input5').value=lastlength
...
document.querySelector(".input6").addEventListener("change", (event) => {
    ...
    document.querySelector('.input6').value=lastwidth
    alert("Введено не коректное значение")
}
});
let find=0.6;
...
document.querySelector(".input7").addEventListener("change", (event) => {
    ...
    document.querySelector('.input7').value=lastheight
    alert("Введено не коректное значение")
}
});
...
document.querySelector('.cover8').addEventListener("click", (event) => {
    console.log(nowType)
    if(nowType=="дверь"){
        var currentHash = window.location.hash;
        // Разбиваем хэш на элементы по символу "$"
        ...
        let hashPartsParts4 = hashParts[13].split('?');
        let hashPartsParts5 = hashParts[14].split('?');

        hashPartsParts1.splice(nowNumber-1, 1);
        hashPartsParts2.splice(nowNumber-1, 1);
        hashPartsParts3.splice(nowNumber-1, 1);
        hashPartsParts4.splice(nowNumber-1, 1);
        hashPartsParts5.splice(nowNumber-1, 1);
        // Собираем все элементы обратно в строку с разделителем "$"
        hashParts[10] = hashPartsParts1.join('?')
        hashParts[11] = hashPartsParts2.join('?')
        hashParts[12] = hashPartsParts3.join('?')
        ...
        let hashPartsParts4 = hashParts[18].split('?');
        let hashPartsParts5 = hashParts[19].split('?');
        ...
        hashPartsParts5.splice(nowNumber-1, 1);
        // Собираем все элементы обратно в строку с разделителем "$"
        hashParts[15] = hashPartsParts1.join('?')
        hashParts[16] = hashPartsParts2.join('?')
        hashParts[17] = hashPartsParts3.join('?')
